...
} gl3_shadow_area_allocator_t;

static gl3_shadow_area_allocator_t alloc;

static void PushFreeArea(int left, int top, int right, int bottom)
{
...
static void RemoveFreeArea(int idx)
{
...
static qboolean SplitArea(allocator_area_t original, allocator_area_t* reserve)
{
                ...
                reserve->bottom > original.top && reserve->top < original.bottom)
        {
                ...
                if (reserve->right < original.right)
                {
                        ...
                        PushFreeArea(newArea.left, newArea.top, newArea.right, newArea.bottom);
                }
...
qboolean GL3_Shadow_Allocate(int width, int height, int* x, int* y)
{
        ...
        for (;;)
        {
                ...
                if (bestIndex == -1)
                {
                        if (alloc.doubleWidth && alloc.width < alloc.maxWidth)
                        {
                                ...
                                {
                                        PushFreeArea(oldWidth, 0, alloc.width, alloc.height);
                                }
                        }
                        else if (!alloc.doubleWidth && alloc.height < alloc.maxHeight)
                        {
                                ...
                                {
                                        PushFreeArea(0, oldHeight, alloc.width, alloc.height);
                                }
                        }
                        else
                        {
        ...
        *y = best->top;
        ...
        for (size_t i = 0; i < alloc.freeAreaCount;)
        {
                if (SplitArea(alloc.freeAreas[i], &reserved))
                {
                        RemoveFreeArea(i);
                }
                else
