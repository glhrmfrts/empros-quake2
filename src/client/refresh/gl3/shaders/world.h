...
static const char* fragmentSrc3Dlm = MULTILINE_STRING(
		...
		void main()
		{
			...
			// apply lightmap
			vec4 lmTex = (texture(lightmap0, passLMcoord)*(1.0f)) * lightstyles[pass_style0];
			lmTex     += (texture(lightmap1, passLMcoord)*(1.0f)) * lightstyles[pass_style1];
			lmTex     += (texture(lightmap2, passLMcoord)*(1.0f)) * lightstyles[pass_style2];
			lmTex     += (texture(lightmap3, passLMcoord)*(1.0f)) * lightstyles[pass_style3];
